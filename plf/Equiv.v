...
Proof.
  ...
  - inversion H; subst; clear H.
    + simpl in H5. apply E_IfFalse.
      rewrite <- (negb_involutive (beval st b)).
...
Proof.
  intros b c Hb.
  split; intros H.
  - apply while_true_nonterm in H. destruct H. assumption.
  - apply while_true_nonterm in H. destruct H. split.
Qed.
...
Lemma t_update_equiv : forall (X : string) st a,
    aeval st <{ X }> = aeval st a ->
    st = (X !-> (aeval st a); st).
Proof.
  intros X st a H.
...
Proof.
  intros X a HX.
  split; intros H.
  - replace st' with (X !-> aeval st a; st).
    apply E_Asgn. reflexivity.
    inversion H; subst.
    symmetry. apply t_update_equiv.
    apply HX.
  - inversion H; subst.
    replace (X !-> aeval st a; st) with st.
    apply E_Skip.
    apply t_update_equiv. apply HX.
Qed.
...
Theorem CSeq_congruence : forall c1 c1' c2 c2',
  ...
  cequiv <{ c1;c2 }> <{ c1';c2' }>.
Proof.
  intros c1 c1' c2 c2' H1 H2. split; intros H.
  - inversion H; subst.
    apply E_Seq with (st':=st'0).
    + apply H1. apply H4.
    + apply H2. apply H7.
...
Theorem CIf_congruence : forall b b' c1 c1' c2 c2',
  ...
  cequiv <{ if b then c1 else c2 end }>
         <{ if b' then c1' else c2' end }>.
Proof.
  intros b b' c1 c1' c2 c2' H1 H2 H3.
  split; intros H.
  - inversion H; subst.
    + apply E_IfTrue.
      ++ rewrite <- H1. apply H8.
