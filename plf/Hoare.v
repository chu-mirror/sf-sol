...
Proof.
  intros m a P.
  intros st st' H1 [H2_1 H2_2]. split.
...
Proof.
  ...
  inversion H1; subst; clear H1.
  exists (st X). split.
  - rewrite t_update_shadow. rewrite t_update_same. apply H2.
...
Proof.
  unfold hoare_triple.
  intros H.
  specialize H with (a := X) (n := 1) (st := (X !-> 1)).
  assert(E1: forall st', (X !-> 1) =[ X := 3; Y := X ]=> st' -> st' Y = 1).
  intros st' Hst'. apply H in Hst'. auto. auto.
  assert(E2: forall st', (X !-> 1) =[ X := 3; Y := X ]=> st' -> st' Y = 3).
  intros st' Hst'. inversion Hst'; subst.
  inversion H2; subst.
  inversion H5; subst.
  simpl. auto.
  assert(E3: (X !-> 1) =[ X := 3; Y := X ]=> (Y !-> 3; X !-> 3; X !-> 1)).
  eapply E_Seq.
  + apply E_Asgn. reflexivity.
  + simpl. apply E_Asgn. reflexivity.
  + specialize E1 with (st' := (Y !-> 3; X !-> 3; X !-> 1)).
    specialize E2 with (st' := (Y !-> 3; X !-> 3; X !-> 1)).
    apply E1 in E3. unfold t_update in E3. simpl in E3.
    discriminate.
Qed.
