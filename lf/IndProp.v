...
(* FILL IN HERE
Inductive clos_equivalent {X: Type} (R: X->X->Prop) : X->X->Prop :=
| t_refl (x : X) : clos_trans R x x
...
Theorem SSSSev__even : forall n,
  ev (S (S (S (S n)))) -> ev n.
Proof.
  intros n H. inversion H as [|n' Hn' Heq].
  inversion Hn' as [|n'' Hn'' Heq'].
  apply Hn''.
Qed.
...
Theorem ev5_nonsense :
  ev 5 -> 2 + 2 = 9.
Proof.
  intros H. inversion H as [| n Hn' Heq].
  inversion Hn' as [| n'' Hn'' Heq'].
  inversion Hn'' as [| n''' Hn''' Heq''].
Qed.
...
Proof.
  ...
  - intros Hm. simpl. apply Hm.
  - intros Hm. apply IHn' in Hm. simpl. apply ev_SS.
    apply Hm.
Qed.
...
Proof.
  ...
  - simpl in Hnm. inversion Hnm. apply IHn' in H0. apply H0.
Qed.
...
Proof.
  ...
  rewrite add_assoc. rewrite (add_comm (n+p) m).
  rewrite (add_assoc m n p). rewrite <- add_assoc.
  assert(L1: ev (p+p)).
  rewrite ev_Even_iff. exists p. rewrite double_plus. reflexivity.
  apply ev_sum. rewrite add_comm. apply Hnm. apply L1.
Qed.
...
Proof.
  ...
  induction Hnm.
  - apply le_n.
  - assert (H: S m <= S (S m)). apply le_S. apply le_n.
    generalize dependent H. apply le_trans. apply IHHnm.
Qed.
...
Proof.
  ...
  inversion H.
  - apply le_n.
  - assert(Hn: n <= S n). apply le_S. apply le_n.
    apply (le_trans n (S n) m). apply Hn. apply H1.
Qed.
...
Proof.
  ...
  - destruct (S n' =? m) eqn:E.
    + rewrite eqb_eq in E. rewrite <- E. right. apply le_n.
...
Theorem le_plus_l : forall a b,
  a <= a + b.
Proof.
  intros a b. induction b as [| n' IHb'].
...
Proof.
  ...
  - assert(L: n1 <= n1 + n2). apply le_plus_l.
    apply(le_trans n1 (n1+n2) m). apply L. apply H.
  - assert(L: n2 <= n1 + n2). rewrite add_comm. apply le_plus_l.
    apply(le_trans n2 (n1+n2) m). apply L. apply H.
Qed.
...
Theorem plus_le_compat_r : forall n m p,
  ...
  n + p <= m + p.
Proof.
  intros n m p H. rewrite add_comm. rewrite (add_comm m p).
...
Theorem le_plus_trans : forall n m p,
  ...
  n <= m + p.
Proof.
  intros n m p H. apply (le_trans n (n+p) (m+p)).
  - apply le_plus_l.
...
Theorem n_lt_m__n_le_m : forall n m,
  ...
  n <= m.
Proof.
  intros n m H. apply (le_trans n (S n) m).
...
Proof.
  intros n1 n2 m H.
  unfold lt in H. split.
  - replace (S(n1+n2)) with ((S n1)+n2) in H.
    apply plus_le in H. destruct H as [H1 H2].
    apply H1. reflexivity.
  - rewrite plus_n_Sm in H. apply plus_le in H.
...
Proof.
  ...
  - intros n H. destruct n as [| n'].
    + reflexivity.
    + assert(L: 1 <= S n'). apply n_le_m__Sn_le_Sm. apply O_le_n.
      assert(contra: 1 <= 0). apply (le_trans 1 (S n') 0).
      apply L. apply H. inversion contra.
  - intros n H. destruct n as [| n'].
    + reflexivity.
    + apply Sn_le_Sm__n_le_m in H. simpl.
      apply IHm'. apply H.
Qed.
...
Proof.
  intros n m o.
  rewrite leb_iff. rewrite leb_iff. rewrite leb_iff.
  apply le_trans.
Qed.
...
Proof.
  ...
  - intros H. induction H.
    ...
    + simpl in IHR. injection IHR as H1.
      rewrite <- plus_n_Sm in H1. injection H1 as H2.
      unfold fR. apply H2.
    + rewrite add_comm. apply IHR.
  - intros H. generalize dependent n. generalize dependent o.
    induction m as [| m' IHm'].
    + intros o. induction o as [| o' IHo'].
      ++ intros n H. simpl in H. rewrite H. apply c1.
      ++ intros n H. destruct n as [| n'].
         +++ simpl in H. discriminate H.
         +++ simpl in H. injection H as H1.
             apply IHo' in H1. apply c3. apply H1.
    + intros o. induction o as [| o' IHo'].
      ++ intros n H. simpl in H. discriminate H.
      ++ intros n H. simpl in H. injection H as H1.
         apply c2. apply IHm'. apply H1.
Qed.
...
| Sub_Nil (l : list nat) : subseq nil l
| Sub_Head (h : nat) (l1 l2 : list nat) (Hsub : subseq l1 l2) :
  subseq (h :: l1) (h :: l2)
| Sub_Tail (h : nat) (l1 l2 : list nat) (Hsub : subseq l1 l2) :
  subseq l1 (h :: l2)
.
...
Proof.
  ...
  induction H.
  - apply Sub_Nil.
  - apply Sub_Head. apply IHsubseq.
  - apply Sub_Tail. apply IHsubseq.
Qed.

Lemma subseq_tail : forall h (l1 l2 : list nat),
    subseq (h :: l1) l2 -> subseq l1 l2.
Proof.
  intros h l1 l2. generalize dependent h. generalize dependent l1.
  induction l2 as [| h' l2' IHl2'].
  - intros l1 h H. inversion H.
  - intros l1 h H. destruct (h =? h') eqn:E.
    + apply eqb_eq in E. rewrite E in H.
      inversion H; subst; clear H.
      ++ apply Sub_Tail. apply Hsub.
      ++ apply IHl2' in Hsub. apply Sub_Tail. apply Hsub.
    + apply Sub_Tail.
      inversion H; subst; clear H.
      ++ apply Hsub.
      ++ apply (IHl2' l1 h). apply Hsub.
Qed.

Lemma subseq_comtail : forall h (l1 l2 : list nat),
    subseq (h :: l1) (h :: l2) -> subseq l1 l2.
Proof.
  intros h l1 l2 H.
  inversion H; subst; clear H.
  - apply Hsub.
  - apply (subseq_tail h). apply Hsub.
Qed.
...
Theorem subseq_trans : forall (l1 l2 l3 : list nat),
  ...
  subseq l1 l3.
Proof.
  intros l1 l2 l3. generalize dependent l1. generalize dependent l3.
  induction l2 as [| h l2' IHl2'].
  - intros l3 l1 H1 H2.
  ...
  - intros l3 l1 H1 H2.
    ...
    + apply Sub_Nil.
    + induction l3 as [| h3 l3' IHl3'].
      ++ inversion H2.
      ++ destruct (h =? h3) eqn:E.
         +++ apply eqb_eq in E. subst.
             apply Sub_Head. apply subseq_comtail in H2.
             apply IHl2'. apply Hsub. apply H2.
         +++ apply Sub_Tail. apply IHl3'.
             inversion H2; subst; clear H2.
             ++++ rewrite eqb_refl in E. discriminate E.
             ++++ apply Hsub0.
    + apply subseq_tail in H2. apply IHl2'.
      apply Hsub. apply H2.
Qed.
...
Proof.
  ...
  - induction re.
    ...
    + intros H. reflexivity.
    + intros [s Hs]. simpl. inversion Hs; subst; clear Hs.
      assert(E1: re_not_empty re1 = true). apply IHre1. exists s1. apply H0.
      assert(E2: re_not_empty re2 = true). apply IHre2. exists s2. apply H3.
      rewrite E1. rewrite E2. reflexivity.
    + intros [s Hs]. inversion Hs; subst; clear Hs.
      ++ simpl. assert(E: re_not_empty re1 = true). apply IHre1.
         exists s. apply H0.
         rewrite E. reflexivity.
      ++ simpl. assert(E: re_not_empty re2 = true). apply IHre2.
         exists s. apply H0.
         rewrite E. apply orb_true_iff. right. reflexivity.
    + intros H. reflexivity.
  - intros H. induction re.
    ...
    + simpl in H. apply andb_true_iff in H.
      ...
      exists (app s1 s2). apply (MApp s1). apply H1.  apply H2.
...
Proof.
  ...
  induction H as [| x
                 ...
                 | re''| s1 s2  re'' H1 IH1 H2 IH2]; try discriminate.
  - exists []. split. reflexivity. intros s' H. simpl in H. destruct H.
  - injection Heqre' as Heqre'. subst.
    destruct IH2 as [ss2 IH2]. reflexivity.
    destruct IH2 as [IH2_1 IH2_2].
    exists (s1 :: ss2). split.
    + simpl. f_equal. apply IH2_1.
