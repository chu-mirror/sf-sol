...
Proof.
  intros P Q H.
  unfold not. intros nQ.
  intros HP.
  apply H in HP. apply nQ in HP.
  destruct HP.
Qed.
...
Proof.
  ...
  intros HnPQ. split.
  - intros HP. assert (HPQ: P \/ Q). left. apply HP.
    apply HnPQ in HPQ. destruct HPQ.
  - intros HQ. assert (HPQ: P \/ Q). right. apply HQ.
    apply HnPQ in HPQ. destruct HPQ.
Qed.
...
Proof.
  ...
  - intros [H1 H2].
    destruct H1 as [HP | HQ]. left. apply HP.
    destruct H2 as [HP' | HR]. left. apply HP'.
...
Proof.
  intros X P.
  intros H1 [x H2].
  unfold not in H2. assert (H: P x). apply H1. apply H2 in H.
  destruct H.
Qed.
...
Proof.
  ...
  - intros m. simpl. intros H. exists m. reflexivity.
  - intros m H.
    induction m as [| m' IHm'].
    + simpl in H. discriminate H.
    + simpl in H. apply IHn' in H.
      destruct H as [x H'].
      exists x. simpl. f_equal. apply H'.
Qed.

Theorem plus_exists_leb : forall n m, (exists x, m = n+x) -> n <=? m = true.
Proof.
  intros n. induction n as [| n' IHn'].
  - intros m [x H]. simpl. reflexivity.
  - intros m [x H]. destruct m as [| m'].
    + discriminate H.
    + injection H as H'. simpl. apply IHn'.
      exists x. apply H'.
Qed.
...
Proof.
  ...
  - induction l as [| v l' IHl'].
    ...
    + simpl. intros H.
      destruct H as [H1 | H2].
      ++ exists v. split. apply H1. left. reflexivity.
      ++ apply IHl' in H2. destruct H2 as [x H2'].
         exists x. destruct H2' as [H2_1 H2_2].
         split. apply H2_1. right. apply H2_2.
  - induction l as [| v l' IHl'].
    + intros [x [H1 H2]]. simpl in H2. destruct H2.
    + intros [x [H1 [H2_1 | H2_2]]].
      ++ simpl. left. rewrite H2_1. apply H1.
      ++ simpl. right. apply IHl'.
         exists x. split. apply H1. apply H2_2.
Qed.
...
Proof.
  ...
  - intros l a. simpl. split. intros H. right. apply H.
    intros [H1 | H2]. destruct H1. apply H2.
  - intros l1 a.
    simpl. rewrite IH. rewrite or_assoc. reflexivity.
Qed.
...
Proof.
  ...
  - induction l as [| v l' IHl'].
    + simpl. intros tr x convo. destruct convo.
    + simpl. intros [H1 H2].
      intros x [H1' | H2'].
      ++ rewrite <- H1'. apply H1.
      ++ apply IHl'. apply H2. apply H2'.
Qed.
...
Proof.
  ...
  - unfold tr_rev. simpl.
    assert(L1: forall (l1 l2: list X), rev_append l1 l2 = (rev_append l1 []) ++ l2).
    intros l1. induction l1 as [| v_l1 l1' IHl1'].
    + reflexivity.
    + simpl. intros l2. rewrite (IHl1' (v_l1::l2)). rewrite (IHl1' [v_l1]).
      rewrite <- app_assoc. simpl. reflexivity.
    + rewrite L1. unfold tr_rev in IHx'. rewrite IHx'. reflexivity.
Qed.
...
Proof.
  intros n. induction n as [| n' [k IHn']].
  - exists 0. reflexivity.
  - destruct (even n') eqn:E.
    + rewrite even_S. rewrite E. simpl. exists k. rewrite IHn'. reflexivity.
    + rewrite even_S. rewrite E. simpl.
      exists (S k). simpl. rewrite IHn'. reflexivity.
Qed.
...
Proof.
  intros b1 b2. split.
  - intros H. split. rewrite andb_commutative in H. apply (andb_true_elim2 b2 b1).
    apply H. apply (andb_true_elim2 b1 b2). apply H.
...
Proof.
  ...
  - intros H. destruct b1 eqn:E.
    + left. reflexivity.
    + right. destruct b2 eqn:E'. reflexivity. discriminate.
  - intros [H1 | H2].
...
Theorem not_false: forall b : bool, b = false <-> ~ (b = true).
Proof.
  intros b. split.
  - intros H. rewrite H. discriminate.
  - destruct b. intros. destruct H. reflexivity.
...
Theorem eqb_neq : forall x y : nat,
  x =? y = false <-> x <> y.
Proof.
  intros x y. rewrite not_false. apply not_iff_compat.
  apply eqb_eq.
Qed.
...
Proof.
  intros A eqb Heqb.
  intros l1. induction l1 as [| v1 l1' IHl1'].
  - intros l2. split.
    + intros H. simpl in H. destruct l2 as [| v l2'].
      reflexivity. discriminate.
    + intros H. rewrite <- H. reflexivity.
  - intros l2.
    destruct l2 as [| v l2'].
    + simpl. split. intros H. discriminate H. intros H. discriminate.
    + simpl. destruct (eqb v1 v) eqn:E.
      apply Heqb in E. split.
      ++ rewrite IHl1'. intros H. rewrite H. rewrite E. reflexivity.
      ++ intros H. injection H as H1 H2. apply IHl1'. apply H2.
      ++ split. intros H. discriminate H. intros H. injection H as H1 H2.
         apply Heqb in H1. rewrite E in H1. discriminate H1.
Qed.
...
Proof.
  ...
  - intros H. unfold All. induction l as [| v l' IHl'].
    + apply I.
    + split. simpl in H. rewrite andb_commutative in H. apply andb_true_elim2 in H.
      apply H. apply IHl'. simpl in H. apply andb_true_elim2 in H. apply H.
  - intros H. induction l as [| v l' IHl'].
    + simpl. reflexivity.
    + simpl. simpl in H. destruct H as [H1 H2].
      rewrite H1. simpl. apply IHl'. apply H2.
Qed.
