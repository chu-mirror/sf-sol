...
Proof.
  ...
  injection H1 as H1_1 H1_2.
  assert (eq1: y :: l = z :: l). transitivity j. apply H1_2. apply H2.
  injection eq1 as eq1_1.
  rewrite H1_1. rewrite eq1_1. reflexivity.
Qed.
...
Proof.
  ...
  - intros m. induction m as [| m' IHm'].
    + intros HC. discriminate HC.
    + intros H. f_equal. apply IHn'. simpl in H. apply H.
Qed.
...
Proof.
  ...
  - intros m eq. induction m as [| m' IHm'].
    + discriminate.
    + f_equal. apply IHn'. simpl in eq. injection eq as eq'.
...
Proof.
  ...
  - intros l1 l2 H.
    simpl in H. injection H as eq1 eq2. rewrite <- eq1. rewrite <- eq2.
    reflexivity.
...
Proof.
  ...
  - destruct (f false) eqn:ff.
    ...
    + destruct b.
      ++ rewrite ft. rewrite ft. apply ft.
      ++ rewrite ff. rewrite ff. apply ff.
  - destruct (f false) eqn:ff.
    + destruct b.
      ++ rewrite ft. rewrite ff. apply ft.
      ++ rewrite ff. rewrite ft. apply ff.
    + destruct b.
...
Proof.
  ...
  - intros m p H1 H2.
    destruct m as [| m'].
    + discriminate H1.
    + simpl in H1. destruct p as [| p'].
      ++ discriminate H2.
      ++ simpl. simpl in H2. generalize dependent H2.
         apply IHn'. apply H1.
Qed.
...
Proof.
  intros X Y l. induction l as [| v l' IHl'].
  - intros l1. induction l1 as [| v1 l1' IHl1'].
    + intros l2 Hlen H. simpl in Hlen. destruct l2 as [| h t].
      ++ reflexivity.
      ++ discriminate Hlen.
    + intros l2 Hlen H. destruct l2 as [| h t].
      ++ discriminate Hlen.
      ++ discriminate H.
  - intros l1. induction l1 as [| v1 l1' IHl1'].
    + intros l2 Hlen H. discriminate H.
    + intros l2 Hlen H. destruct l2 as [| h t].
      ++ discriminate H.
...
Theorem filter_exercise : forall (X : Type) (test : X -> bool)
  ...
  test x = true.
Proof.
  intros X test x l lf H. destruct (test x) eqn:E.
  + reflexivity.
  + induction l as [| v l' IHl'].
    ++ discriminate H.
    ++ destruct (test v) eqn:E'.
       +++ simpl in H. rewrite E' in H. injection H as H1 H2.
           rewrite H1 in E'. rewrite E in E'. discriminate E'.
       +++ simpl in H. rewrite E' in H. apply IHl'. apply H.
Qed.
...
Proof.
  intros X test l. induction l as [| v l' IHl'].
  - reflexivity.
  - simpl. unfold existsb'.
    simpl. unfold existsb' in IHl'.
    assert(forall b1 b2, negb(b1 && b2) = (negb b1) || (negb b2)).
    intros b1 b2. destruct b1.
    + destruct b2. reflexivity. reflexivity.
    + destruct b2. reflexivity. reflexivity.
    + rewrite H. rewrite <- IHl'. rewrite negb_involutive. reflexivity.
Qed.
