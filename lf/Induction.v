...
Proof.
  ...
  - rewrite -> add_0_r. reflexivity.
  - simpl. rewrite <- plus_n_Sm. rewrite -> IHm'. rewrite <- plus_n_Sm. reflexivity.
Qed.
...
Proof.
  ...
  - assert (mult_n_0: m * 0 = 0).
      induction m as [ | m' IHm']. reflexivity. simpl. rewrite -> IHm'. reflexivity.
      rewrite -> mult_n_0. reflexivity.
  - assert (mult_n_Sm: forall n m: nat, n * S m = n + n * m).
    intros n1 m1. induction n1 as [| n1' IHn1']. reflexivity. simpl. rewrite -> IHn1'. rewrite -> add_shuffle3. reflexivity.
    rewrite -> mult_n_Sm. simpl. rewrite -> IHn'. reflexivity.
Qed.
...
Proof.
  ...
  rewrite IHp'. rewrite (add_assoc (n*p'+m*p') n m).
  rewrite (add_assoc (n*p'+n) (m*p') m).
  rewrite <- (add_assoc (n*p') (m*p') n).
  rewrite <- (add_assoc (n*p') n (m*p')).
...
Proof.
  ...
  - reflexivity.
  - reflexivity.
  - simpl. rewrite IHb2. simpl. rewrite <- plus_n_Sm. reflexivity.
Qed.
...
(* FILL IN HERE *)
Example normal_1 : normalize (B0 Z) = Z.
Proof. reflexivity. Qed.
...
Lemma nat_bin_double :
  forall n, nat_to_bin(2*n) = double_bin (nat_to_bin n).
Proof.
  intros n. induction n as [| n' IHn'].
  - reflexivity.
  - rewrite <- mult_n_Sm. rewrite add_comm. simpl. simpl in IHn'.
...
Proof.
  intros b. induction b as [| b1 IHb1 | b2 IHb2].
  - reflexivity.
  - simpl. rewrite add_0_r. rewrite <- mult_2. rewrite nat_bin_double.
    f_equal. apply IHb1.
  - simpl. rewrite add_0_r. rewrite <- mult_2. rewrite nat_bin_double.
    rewrite IHb2. destruct (normalize b2) as [| b21 | b22].
    + reflexivity.
    + simpl. reflexivity.
    + simpl. reflexivity.
Qed.
